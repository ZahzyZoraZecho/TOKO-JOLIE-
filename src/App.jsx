...
function JolieCustomerServiceWidget(){
 const [open,setOpen]=React.useState(false),[q,setQ]=React.useState(""),[messages,setMessages]=React.useState([{role:"ai",text:"Halo, selamat datang di JOLIE 😊 Ada yang bisa saya bantu hari ini?"}]),[busy,setBusy]=React.useState(false),[conversation,setConversation]=React.useState(null);
 async function send(){if(!q.trim()||busy)return;const text=q.trim();setQ("");setMessages(m=>[...m,{role:"user",text}]);setBusy(true);const r=await supabase.functions.invoke("jolie-ai-chat",{body:{widget_key:"1bee973ccf64e827bebcb0b4",message:text,conversation_id:conversation,mode:"website_member",visitor_id:localStorage.getItem("jolie_visitor_id")||""}});setBusy(false);if(r.data?.conversation_id){setConversation(r.data.conversation_id);try{localStorage.setItem("jolie_visitor_id",r.data.conversation_id)}catch{}}setMessages(m=>[...m,{role:"ai",text:r.error?.message||r.data?.reply||"Maaf, saya sedang menyiapkan jawaban. Silakan coba lagi sebentar."}])}
 return <><button aria-label="JOLIE Customer Service" onClick={()=>setOpen(!open)} style={{position:"fixed",right:20,bottom:20,zIndex:1200,width:58,height:58,borderRadius:"50%",border:0,background:"#08764d",color:"#fff",boxShadow:"0 10px 30px rgba(0,0,0,.2)",display:"grid",placeItems:"center",cursor:"pointer"}}><MessageCircle/></button>{open&&<div style={{position:"fixed",right:20,bottom:88,zIndex:1200,width:"min(380px,calc(100vw - 32px))",height:520,borderRadius:20,background:"#fff",boxShadow:"0 20px 60px rgba(0,0,0,.25)",overflow:"hidden",border:"1px solid #dce9e2",display:"flex",flexDirection:"column"}}><div style={{padding:"16px 18px",background:"#08764d",color:"#fff"}}><b>JOLIE Customer Service</b><div style={{fontSize:12,opacity:.85}}>Hangat · Sopan · Profesional</div></div><div style={{flex:1,overflow:"auto",padding:14}}>{messages.map((m,i)=><div key={i} style={{marginBottom:10,textAlign:m.role==="user"?"right":"left"}}><span style={{display:"inline-block",maxWidth:"88%",padding:"10px 12px",borderRadius:14,background:m.role==="user"?"#edf7f1":"#f4f5f4",fontSize:14}}>{m.text}</span></div>)}{busy&&<div style={{fontSize:12,color:"#667"}}>JOLIE sedang menyiapkan jawaban…</div>}</div><form onSubmit={e=>{e.preventDefault();send()}} style={{display:"flex",gap:8,padding:12,borderTop:"1px solid #eee"}}><input value={q} onChange={e=>setQ(e.target.value)} placeholder="Tulis pesan…" style={{flex:1,border:"1px solid #ccd8d1",borderRadius:12,padding:"10px 12px"}}/><button type="submit" disabled={busy} style={{border:0,borderRadius:12,padding:"0 14px",background:"#08764d",color:"#fff"}}><Send size={17}/></button></form></div>}</>
}
function App() {
...
`;
  ...
  <JolieCustomerServiceWidget/>{showTop&&<button className="back-top" onClick={()=>window.scrollTo({top:0,behavior:"smooth"})}><ArrowUp size={18}/></button>}{cart.length>0&&<div className="sticky-cart"><div><ShoppingCart size={17}/><b>{cartCount} item</b><span>{money(cartTotal)}</span></div><button onClick={()=>setCartOpen(true)}>Lihat Keranjang</button></div>}
  </div>;
}
export default App;
