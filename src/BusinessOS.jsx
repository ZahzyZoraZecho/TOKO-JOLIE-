...
import {
  ArrowLeft, BarChart3, Boxes, BrainCircuit, CircleDollarSign, ClipboardList, ShoppingCart,
...
export default function BusinessOS({ user, onBack }) {
  ...
  const moduleStats = {
    ...
    pos: { title: "POS Kasir", subtitle: "Transaksi kasir, pembayaran dan idempotency yang aman" },
  ...
  return <div className="bos-shell">
    <style>{`
      ...
      .bos-module-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:10px}.bos-pos{display:grid;grid-template-columns:minmax(0,1.3fr) minmax(300px,.7fr);gap:12px}.bos-pos-search{width:100%;border:1px solid #dce7e1;border-radius:9px;padding:10px 12px;font-size:10px;box-sizing:border-box}.bos-pos-products{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-top:10px}.bos-pos-product{border:1px solid #dce7e1;background:#fff;border-radius:10px;padding:11px;text-align:left;cursor:pointer}.bos-pos-product:hover{border-color:#08764d}.bos-pos-product b{display:block;font-size:10px;margin-top:4px}.bos-pos-product span{display:block;font-size:8px;color:#71837c;margin-top:4px}.bos-pos-cart{background:#fff;border:1px solid #dce7e1;border-radius:13px;padding:14px}.bos-pos-row{display:grid;grid-template-columns:1fr 65px 75px;gap:6px;align-items:center;padding:9px 0;border-bottom:1px solid #eef2f0;font-size:9px}.bos-pos-qty{display:flex;align-items:center;gap:4px}.bos-pos-qty button{width:22px;height:22px;border:1px solid #dce7e1;background:#fff;border-radius:5px}.bos-pos-total{display:flex;justify-content:space-between;font-size:11px;font-weight:800;margin-top:12px}.bos-pos-pay{display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-top:10px}.bos-pos-pay button{border:1px solid #dce7e1;background:#fff;border-radius:7px;padding:8px;font-size:8px;font-weight:800}.bos-pos-pay button.active{background:#08764d;color:#fff;border-color:#08764d}.bos-pos-submit{width:100%;border:0;background:#08764d;color:#fff;border-radius:8px;padding:11px;margin-top:10px;font-weight:800}.bos-pos-message{margin-top:9px;padding:9px;border-radius:8px;background:#edf7f2;color:#356754;font-size:8px;line-height:1.5}.bos-module-card{border:1px solid #dce7e1;background:#fff;border-radius:12px;padding:14px;text-align:left}.bos-module-card svg{color:#08764d}.bos-module-card b{display:block;font-size:11px;margin-top:8px}.bos-module-card span{display:block;font-size:8px;color:#71837c;line-height:1.5;margin-top:4px}.bos-module-card button{margin-top:10px;border:0;background:#edf7f2;color:#08764d;border-radius:7px;padding:7px 9px;font-size:8px;font-weight:800}
    ...
    <div className="bos-layout">
      ...
      <main className="bos-main">
        ...
        {loading ? <div className="bos-muted">Memuat data operasional JOLIE…</div> : <>
          {active==="pos" && <POS products={products} customers={customers} role={role} onRefresh={loadData}/>}
...
function TeamAccess({organizationId,role}){
  ...
  const load=React.useCallback(async()=>{
    const [rr,mm]=await Promise.all([
      ...
      businessSupabase.from("organization_members").select("id,user_id,role,is_active,created_at").eq("organization_id",organizationId).order("created_at")
    ]);
...
function POS({products,customers,role,onRefresh}){
  const [cart,setCart]=React.useState([]); const [search,setSearch]=React.useState(""); const [method,setMethod]=React.useState("cash"); const [customer,setCustomer]=React.useState(""); const [busy,setBusy]=React.useState(false); const [message,setMessage]=React.useState("");
  const available=products.filter(p=>p.is_active&&p.price!=null).filter(p=>!search||String(p.name).toLowerCase().includes(search.toLowerCase())||String(p.sku||"").toLowerCase().includes(search.toLowerCase())).slice(0,18);
  const total=cart.reduce((s,x)=>s+Number(x.price)*x.quantity,0);
  const add=(p)=>setCart(prev=>{const e=prev.find(x=>x.id===p.id);return e?prev.map(x=>x.id===p.id?{...x,quantity:x.quantity+1}:x):[...prev,{id:p.id,name:p.name,price:Number(p.price),quantity:1}]});
  const change=(id,d)=>setCart(prev=>prev.map(x=>x.id===id?{...x,quantity:Math.max(1,x.quantity+d)}:x));
  const remove=(id)=>setCart(prev=>prev.filter(x=>x.id!==id));
  const checkout=async()=>{if(!cart.length||busy)return;setBusy(true);setMessage("");const key=crypto.randomUUID();const {data,error}=await businessSupabase.rpc("jolie_pos_create_sale",{p_items:cart.map(x=>({product_id:x.id,quantity:x.quantity})),p_payment_method:method,p_amount:total,p_customer_id:customer||null,p_idempotency_key:key});if(error){setMessage(error.message||"Transaksi POS gagal.");}else{setMessage("Transaksi berhasil: "+(data?.[0]?.order_number||"POS"));setCart([]);if(onRefresh)onRefresh();}setBusy(false);};
  return <div className="bos-pos"><section className="bos-panel"><div className="bos-panel-head"><h2>Pilih produk</h2><span>{products.filter(p=>p.is_active).length} aktif · role {role}</span></div><input className="bos-pos-search" value={search} onChange={e=>setSearch(e.target.value)} placeholder="Cari nama produk atau SKU…"/><div className="bos-pos-products">{available.map(p=><button className="bos-pos-product" key={p.id} onClick={()=>add(p)}><ShoppingBag size={17}/><b>{p.name}</b><span>{rupiah(p.price)} · {p.unit||"unit"}</span></button>)}</div>{!available.length&&<div className="bos-muted" style={{marginTop:10}}>Tidak ada produk berharga yang dapat dijual.</div>}</section><aside className="bos-pos-cart"><div className="bos-panel-head"><h2>Keranjang Kasir</h2><span>{cart.length} item</span></div>{customers.length>0&&<select className="bos-pos-search" value={customer} onChange={e=>setCustomer(e.target.value)}><option value="">Walk-in / tanpa pelanggan</option>{customers.map(c=><option key={c.id} value={c.id}>{c.name||c.phone||c.email}</option>)}</select>}{cart.length?cart.map(x=><div className="bos-pos-row" key={x.id}><div><b>{x.name}</b><div>{rupiah(x.price)}</div></div><div className="bos-pos-qty"><button onClick={()=>change(x.id,-1)}>−</button><span>{x.quantity}</span><button onClick={()=>change(x.id,1)}>+</button></div><button onClick={()=>remove(x.id)} style={{border:0,background:"transparent",color:"#a04a42"}}>Hapus</button></div>):<div className="bos-muted">Belum ada item.</div>}<div className="bos-pos-total"><span>Total</span><span>{rupiah(total)}</span></div><div className="bos-pos-pay">{[["cash","Cash"],["qris","QRIS"],["bank_transfer","Transfer"],["edc","EDC"]].map(([id,label])=><button key={id} className={method===id?"active":""} onClick={()=>setMethod(id)}>{label}</button>)}</div><button className="bos-pos-submit" disabled={!cart.length||busy} onClick={checkout}>{busy?"Memproses…":"Selesaikan Transaksi"}</button>{message&&<div className="bos-pos-message">{message}</div>}</aside></div>}
